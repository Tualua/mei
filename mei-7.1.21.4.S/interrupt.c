...
static void _mei_cmpl_iamthif(struct mei_device *dev, struct mei_cl_cb *cb_pos)
{
	if (dev->iamthif_canceled != 1) {
		...
		list_add_tail(&cb_pos->cb_list,	&dev->amthi_read_complete_list.mei_cb.cb_list);
		...
		struct mei_msg_hdr *mei_hdr)
{
	...
	buffer = (unsigned char *) (dev->iamthif_msg_buf +	dev->iamthif_msg_buf_index);
...
static int _mei_irq_thread_state_ok(struct mei_cl *cl,
				struct mei_msg_hdr *mei_hdr)
{
	return (cl->host_client_id == mei_hdr->host_addr &&	cl->me_client_id == mei_hdr->me_addr &&
			MEI_FILE_CONNECTED == cl->state && MEI_READ_COMPLETE != cl->reading_state);
}
		...
		struct mei_msg_hdr *mei_hdr)
{
	...
	if (!(dev->read_list.status == 0 && !list_empty(&dev->read_list.mei_cb.cb_list)))
		goto quit;

	list_for_each_entry_safe(cb_pos, cb_next, &dev->read_list.mei_cb.cb_list, cb_list) {
		cl = (struct mei_cl *)cb_pos->file_private;
		if (cl && _mei_irq_thread_state_ok(cl, mei_hdr)) {
			cl->reading_state = MEI_READING;
			buffer = (unsigned char *)(cb_pos->response_buffer.data + cb_pos->information);

			if (cb_pos->response_buffer.size < mei_hdr->length + cb_pos->information) {
...
{
	if (cl->host_client_id == rs->host_addr && cl->me_client_id == rs->me_addr)
	{
		if (!rs->status)
		{
		...
		struct hbm_client_connect_response *rs)
{
	...
	if (!dev->ctrl_rd_list.status && !list_empty(&dev->ctrl_rd_list.mei_cb.cb_list))
	{
...
{
	return (cl->host_client_id == flow->host_addr && cl->me_client_id == flow->me_addr);
}
...
{
	return (cl->host_client_id == disconn->host_addr && cl->me_client_id == disconn->me_addr);
}
		...
		struct hbm_flow_control *flow_control)
{
	...
	if (!flow_control->host_addr) {
		...
		/* normal connection */
		list_for_each_entry(cl_pos, &dev->file_list, link) {
			dev_dbg(&dev->pdev->dev, "list_for_each_entry in file_list\n");
		...
		struct hbm_client_disconnect_request *disconnect_req)
{
	...
	list_for_each_entry(cl_pos, &dev->file_list, link) {
		if (same_disconn_addr(cl_pos, disconnect_req)) {
			...
			mei_hdr->host_addr = 0;
			mei_hdr->me_addr = 0;
			mei_hdr->length = sizeof(struct hbm_client_connect_response);
			mei_hdr->msg_complete = 1;
			mei_hdr->reserved = 0;

			disconnect_res = (struct hbm_client_connect_response *)&dev->ext_msg_buf[1];
			...
			struct mei_io_list *cmpl_list)
{
	...
	if ((*slots * sizeof(u32)) >= (sizeof(struct mei_msg_hdr) +
			(cb_pos->request_buffer.size - cb_pos->information))) {
		...
		dev_dbg(&dev->pdev->dev, "mei_hdr->length  =%d\n",
				mei_hdr->length);
		*slots -= (sizeof(struct mei_msg_hdr) +	mei_hdr->length + 3) / 4;
		if (!mei_write_message(dev, mei_hdr,
			...
			cl->status = 0;
			cb_pos->information += mei_hdr->length;
			list_move_tail(&cb_pos->cb_list, &dev->write_waiting_list.mei_cb.cb_list);
		}
		...
		mei_hdr->host_addr = cl->host_client_id;
		mei_hdr->me_addr = cl->me_client_id;
		mei_hdr->length = (*slots * sizeof(u32)) - sizeof(struct mei_msg_hdr);
		...
		s32 *slots)
{
	...
	if (mei_hdr->host_addr || mei_hdr->me_addr) {
		list_for_each_entry(cl_pos, &dev->file_list, link) {
			dev_dbg(&dev->pdev->dev,
					"list_for_each_entry read host client = %d,"
					" ME client = %d\n",
					cl_pos->host_client_id,	cl_pos->me_client_id);

			if (cl_pos->host_client_id == mei_hdr->host_addr && cl_pos->me_client_id == mei_hdr->me_addr)
				break;
		}//for each
	...
	if (((*slots) * sizeof(u32)) < mei_hdr->length) {
		dev_dbg(&dev->pdev->dev, "we can't read the message slots =%08x.\n",	*slots);
	...
	if (!mei_hdr->host_addr) {
		...
		dev_dbg(&dev->pdev->dev, "mei_hdr->length =%d\n", mei_hdr->length);
		ret = mei_irq_thread_read_amthi_message(cmpl_list, dev, mei_hdr);
		if (ret)
			goto end;
	} else {
		dev_dbg(&dev->pdev->dev, "call mei_irq_thread_read_client_message.\n");
		ret = mei_irq_thread_read_client_message(cmpl_list, dev, mei_hdr);
		if (ret)
			goto end;
	}
 ...
 * returns 0 on success, <0 on failure.
 */
static int mei_irq_thread_write_handler(
		struct mei_io_list *cmpl_list,
		struct mei_device *dev,
		s32 *slots)
{
	...
	*slots = mei_count_empty_write_slots(dev);
	...
	if (!list->status && !list_empty(&list->mei_cb.cb_list)) {
		list_for_each_entry_safe(cb_pos, cb_next, &list->mei_cb.cb_list, cb_list) {
			cl = (struct mei_cl *)cb_pos->file_private;
			if (!cl)
				continue;
			cl->status = 0;
			...
			if (MEI_WRITING == cl->writing_state &&
				   (cb_pos->major_file_operations == MEI_WRITE) &&
				   (cl != &dev->iamthif_cl)) {
	...
	if (!dev->ctrl_wr_list.status) {
		...
		list_for_each_entry_safe(cb_pos, cb_next,
			...
			default:
				dev_dbg(&dev->pdev->dev,"inconsistent messsage");
				return -EBADMSG;
			}
	...
	if (!dev->write_list.status && !list_empty(&dev->write_list.mei_cb.cb_list)) {
		...
		list_for_each_entry_safe(cb_pos, cb_next,
			...
			if (cl != &dev->iamthif_cl) {
				if (!mei_flow_ctrl_creds(dev, cl)) {
					dev_dbg(&dev->pdev->dev,
							"No flow control credentials for client"
							" %d, not sending.\n",
							cl->host_client_id);
					continue;
				}
				ret = _mei_irq_thread_cmpl(dev, slots, cb_pos, cl, cmpl_list);
				if (ret)
					return ret;
				...
				if (!mei_flow_ctrl_creds(dev, cl)) {
					dev_dbg(&dev->pdev->dev,
							"No flow control credentials for amthi"
							" client %d.\n",
							cl->host_client_id);
					continue;
				}
				ret = _mei_irq_thread_cmpl_iamthif(dev,	slots, cb_pos, cl,
						cmpl_list);
				if (ret)
					return ret;
			}
...
void mei_wd_timer(struct work_struct *work)
{
	...
	struct mei_device *dev = container_of(work,	struct mei_device, wd_work. work);
	...
	if (dev->iamthif_timer) {
		timeout = dev->iamthif_timer + msecs_to_jiffies(IAMTHIF_READ_TIMER);
		...
		if (time_after(jiffies, timeout)) {
			...
			{
				list_for_each_entry_safe(cb_pos, cb_next, amthi_complete_list,
							cb_list) {
...
irqreturn_t mei_interrupt_thread_handler(int irq, void *dev_id)
{
	...
	//possible extra approach if (rets < 0) can reseting whole communications
	...
	{
		list_for_each_entry_safe(cb_pos, cb_next, &complete_list.mei_cb.cb_list, cb_list)
		{
			cl = (struct mei_cl *)cb_pos->file_private;
			list_del(&cb_pos->cb_list);
			if (!cl)
