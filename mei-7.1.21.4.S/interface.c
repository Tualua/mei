			     ...
			     unsigned long write_length)
{
	...
	dev_dbg(&dev->pdev->dev, "host_hw_state = 0x%08x.\n",
			dev->host_hw_state);

	dev_dbg(&dev->pdev->dev, "mei_write_message header=%08x.\n",
			*((u32 *) header));

	buffer_depth = (unsigned char) ((dev->host_hw_state & H_CBD) >> 24);
	filled_slots = _host_get_filled_slots(dev);
	empty_slots = buffer_depth - filled_slots;
	dev_dbg(&dev->pdev->dev, "filled = %hu, empty = %hu.\n",
	...
	while (write_length >= 4) {
		mei_reg_write(dev, H_CB_WW,	*(u32 *) (write_buffer + bytes_written));
		bytes_written += 4;
		write_length -= 4;
	}
		     ...
		     unsigned char *buffer, unsigned long buffer_length)
{
	...
	while (buffer_length >= sizeof(u32)) {
		...
		dev_dbg(&dev->pdev->dev, "buffer[%d]= %d\n", i, ((u32 *) buffer)[i]);
...
//	memset(mei_flow_control->reserved, 0, sizeof(mei_flow_control->reserved));

	dev_dbg(&dev->pdev->dev, "sending flow control host client = %d, ME client = %d\n",
	    cl->host_client_id, cl->me_client_id);

	if (!mei_write_message(dev, mei_hdr, (unsigned char *) mei_flow_control,
...
int mei_disconnect(struct mei_device *dev, struct mei_cl *cl)
{
	...
	mei_cli_disconnect = (struct hbm_client_disconnect_request *) &dev->wr_msg_buf[1];
...
int mei_connect(struct mei_device *dev, struct mei_cl *cl)
{
	...
	mei_cli_connect = (struct hbm_client_connect_request *) &dev->wr_msg_buf[1];
