...
static const char mei_driver_string[] = "Intel(R) Management Engine Interface for Server";
...
static const struct pci_device_id mei_pci_tbl[] = {
		...
		struct mei_cl *cl)
{
	...
	if (!dev->read_list.status &&
		...
		list_for_each_entry(cb_pos, &dev->read_list.mei_cb.cb_list,
				cb_list) {
...
MODULE_DESCRIPTION("Intel(R) Management Engine Interface for Server");
